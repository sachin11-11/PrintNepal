...
export async function POST(request: NextRequest) {
  ...
  const { data: orderResult, error } = await supabase
    ...
    .insert({
      ...
      estimated_completion_minutes: completionMinutes,
      paper_size: body.paper_size || template.category || "Custom",
  ...
  const whatsappLink = buildWhatsAppLink(process.env.NEXT_PUBLIC_SHOP_WHATSAPP_NUMBER, {
    ...
    productName: order.services?.title ?? "Print product",
    templateName: order.product_templates?.title,
