...
export function AdminLoginForm({ nextPath }: { nextPath?: string }) {
  ...
  return (
    <form className="print-panel w-full p-6 sm:p-8" onSubmit={handleSubmit}>
      ...
      <p className="mt-3 text-sm leading-6 text-graphite">Use a Supabase Auth user with `profiles.role = admin`.</p>
      <label className="mt-8 grid gap-2 text-sm font-medium text-ink">
        Email
        <input className="min-h-12 border border-ink/15 px-4 outline-none focus:border-press" onChange={(event) => setEmail(event.target.value)} placeholder="admin@printnepal.com" required type="email" value={email} />
      </label>
      <label className="mt-5 grid gap-2 text-sm font-medium text-ink">
        Password
        <input className="min-h-12 border border-ink/15 px-4 outline-none focus:border-press" onChange={(event) => setPassword(event.target.value)} placeholder="Password" required type="password" value={password} />
      </label>
