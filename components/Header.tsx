...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-ink/15 bg-paper/95 backdrop-blur-xl">
      <div className="border-b border-ink/10 bg-[var(--solid)] text-[var(--solid-text)]">
      ...
      <div className="mx-auto max-w-7xl px-5 py-3 sm:px-8 lg:px-10">
        <div className="flex items-center justify-between gap-3">
          ...
          <nav className="hidden items-center border border-ink/10 bg-[var(--surface)] text-sm font-semibold text-graphite lg:flex" aria-label="Primary navigation">
            {navItems.map((item) => (
              <a
          ...
          <div className="flex items-center gap-2">
            <ThemeSwitcher />
            <a href="/cart" className="inline-flex min-h-11 items-center gap-3 border border-ink bg-[var(--surface)] px-4 text-sm font-bold text-ink transition hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-soft" aria-label="Open cart">
              <span aria-hidden="true" className="relative flex h-5 w-5 items-center justify-center">
                ...
                <span className="absolute -right-2 -top-2 flex h-4 min-w-4 items-center justify-center bg-ink px-1 text-[10px] leading-none text-paper">
                  0
                </span>
              </span>
        ...
        <nav className="mt-3 flex gap-2 overflow-x-auto border-t border-ink/10 pt-3 text-sm font-semibold lg:hidden" aria-label="Mobile navigation">
          {navItems.map((item) => (
            <a className="shrink-0 border border-ink/10 bg-[var(--surface)] px-3 py-2 text-graphite" href={item.href} key={item.href}>
              {item.label}
            </a>
          ))}
        </nav>
