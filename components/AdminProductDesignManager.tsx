...
export function AdminProductDesignManager() {
  ...
  return (
    <section>
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.28em] text-graphite">Design catalog</p>
        <h1 className="mt-4 font-serif text-5xl text-ink">Product design options.</h1>
      ...
      <form className="mt-8 grid gap-4 rounded-[1.5rem] border border-black/10 bg-white p-5 shadow-sm lg:grid-cols-3" onSubmit={handleSubmit}>
        <select className="min-h-11 rounded-full border border-black/10 bg-white px-4" onChange={(event) => setForm((current) => ({ ...current, service_id: event.target.value }))} required value={form.service_id}>
          <option value="" disabled>Select product category</option>
          {services.map((service) => (
            <option key={service.id} value={service.id}>{service.title}</option>
          ))}
        </select>
        <input className="min-h-11 rounded-full border border-black/10 px-4" onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} placeholder="Design name" required value={form.title} />
        <input className="min-h-11 rounded-full border border-black/10 px-4" onChange={(event) => setForm((current) => ({ ...current, category: event.target.value }))} placeholder="Style/category" value={form.category} />
        <input className="min-h-11 rounded-full border border-black/10 px-4" min="0" onChange={(event) => setForm((current) => ({ ...current, price: event.target.value }))} placeholder="Price" required type="number" value={form.price} />
        <input className="min-h-11 rounded-full border border-black/10 px-4 lg:col-span-2" onChange={(event) => setForm((current) => ({ ...current, image_url: event.target.value }))} placeholder="Image URL" value={form.image_url} />
        <label className="flex min-h-11 cursor-pointer items-center justify-center rounded-full border border-black/10 px-4 text-sm font-medium text-ink">
          Upload design image
        ...
        <textarea className="min-h-28 rounded-3xl border border-black/10 p-4 lg:col-span-3" onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} placeholder="Design details, materials, color choices" value={form.description} />
        <button className="min-h-11 rounded-full bg-ink px-5 text-sm font-medium text-white disabled:bg-neutral-400 lg:col-span-3" disabled={isSaving} type="submit">
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {designs.map((design) => (
          <article key={design.id} className="overflow-hidden rounded-[1.5rem] border border-black/10 bg-white shadow-sm">
            {design.image_url ? (
            ...
            <div className="p-5">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-graphite">{design.services?.title ?? "Product"}</p>
              <h2 className="mt-2 text-lg font-semibold text-ink">{design.title}</h2>
